...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch("http://192.168.1.208:3000/register", {
        method: "POST",
      ...
      if (response.ok) {
        alert(data.message);
        setRegisterData({ email, password }); // Guardar email en el contexto para usarlo en actualizaciones posteriores [MODIFICADO]
        navigation.navigate("FormProfile"); // Se navega a la siguiente pantalla [MODIFICADO]
      } else {
        alert(data.message);
      }
    } catch (error) {
